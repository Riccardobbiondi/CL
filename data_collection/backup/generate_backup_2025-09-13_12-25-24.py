...
class DatasetGenerator:
    def __init__(self):
        ...
        try:
            from segmentation_tools.realtime_object_detector import RealtimeObjectDetector
            self.object_detector = RealtimeObjectDetector()
            print("✅ Object detector avanzato inizializzato")
        except ImportError:
        ...
        try:
            ...
            if len(responses) == 2 and all(len(r.image_data_uint8) > 0 for r in responses):
                ...
                try:
                    ...
                    # NUOVO: Se abbiamo object detector, usa quello invece della segmentazione AirSim
                    if self.object_detector is not None:
                        print("[DEBUG] Usando object detector avanzato...")
                        detection_result = self.object_detector.process_airsim_image(scene_img)
                        semantic_mask = detection_result['semantic_mask']
                        
                        # Converti la maschera semantica nel formato che si aspetta il resto del codice
                        # (array 2D con valori grigi corrispondenti alle categorie)
                        seg_array = self.convert_semantic_to_segmentation(semantic_mask, detection_result)
                        
                        print(f"[DEBUG] Object detector completato, unique values: {len(np.unique(seg_array))}")
                        return scene_img, seg_array
                    
                    # Segmentation image AirSim standard
