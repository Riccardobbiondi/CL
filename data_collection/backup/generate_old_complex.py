...
Genera            from segmentation_tools.simple_detector import SimpleObjectDetector
            object_detector = SimpleObjectDetector()artelle anchor_XXXXX con immagine anchor e 6 positivi
"""
...
class DatasetGenerator:
    def __init__(self):
        ...
        # Inizializza detector oggetti (opzionale, per future implementazioni)
        self.object_detector = None
        try:
            from segmentation_tools.simple_detector import SimpleObjectDetector
            self.object_detector = SimpleObjectDetector()
            print("✅ Object detector semplificato inizializzato")
        except ImportError:
            print("ℹ️ Object detector non disponibile - usando segmentazione AirSim standard")
        except Exception as e:
            print(f"⚠️ Errore inizializzazione object detector: {e}")
            print("ℹ️ Usando segmentazione AirSim standard")
    ...
    def capture_with_segmentation(self):
        """Cattura con logica SEMPLIFICATA come capture_images.py"""
        try:
            print("[DEBUG] Tentativo cattura semplificata...")
            
            # 1. Cattura scene normale
            scene_responses = self.client.simGetImages([
                airsim.ImageRequest("0", airsim.ImageType.Scene, False, True)
            ])
            
            if not scene_responses or len(scene_responses[0].image_data_uint8) == 0:
                print("[DEBUG] Nessuna scene image ricevuta")
                return None, None
                
            # Processa scene image
            scene_bytes = scene_responses[0].image_data_uint8
            scene_img = Image.open(BytesIO(scene_bytes)).convert("RGB")
            scene_img = scene_img.resize(IMG_SIZE, Image.LANCZOS)
            print(f"[DEBUG] Scene caricata: {scene_img.size}")
            
            # 2. Cattura segmentation mask SEMPLICE (come capture_images.py)
            seg_responses = self.client.simGetImages([
                airsim.ImageRequest("0", airsim.ImageType.Segmentation, False, True)
            ])
            
            if seg_responses and len(seg_responses[0].image_data_uint8) > 0:
                # LOGICA SEMPLICE: prendi la mask grezza e binarizzala
                seg_bytes = seg_responses[0].image_data_uint8
                mask = Image.open(BytesIO(seg_bytes)).convert("L").resize(IMG_SIZE, Image.NEAREST)
                ...
                print(f"[DEBUG] Valori unici nella maschera: {unique_values}")
                
                # BINARIZZA: tutto > 10 = ostacolo (255), resto = cielo (0)
                mask_binary = mask.point(lambda x: 255 if x > 10 else 0)
                
                # Converti in formato compatibile con il resto del codice
                # Usa i nostri valori di categoria standard
                seg_array = np.array(mask_binary)
                
                # Mappa: 0 = cielo (5), 255 = ostacoli (50)
                final_mask = np.where(seg_array > 128, 50, 5)  # ostacoli = trees value
                
                print(f"[DEBUG] Mask semplificata: cielo={np.sum(final_mask==5)}, ostacoli={np.sum(final_mask==50)}")
                
                return scene_img, final_mask
            else:
                print("[DEBUG] Nessuna segmentation mask ricevuta")
                return scene_img, None
                
        except Exception as e:
            print(f"[ERROR] Errore cattura semplificata: {e}")
            return None, None
            ...
            # NUOVO: Se abbiamo object detector, usa quello
            if self.object_detector is not None:
                print("[DEBUG] Usando object detector avanzato su raw image...")
                detection_result = self.object_detector.process_airsim_image(scene_img)
                semantic_mask = detection_result['semantic_mask']
                seg_array = self.convert_semantic_to_segmentation(semantic_mask, detection_result)
                print(f"[DEBUG] Object detector su raw completato, unique values: {len(np.unique(seg_array))}")
    ...
    def convert_semantic_to_segmentation(self, semantic_mask, detection_result):
        """Converte maschera semantica dell'object detector nel formato segmentazione AirSim"""
        try:
            # Mappa le categorie semantiche ai valori grigi che si aspetta il resto del codice
            category_mapping = detection_result['category_mapping']
            
            # Crea array di output nel formato aspettato (valori grigi 0-255)
            seg_array = np.zeros_like(semantic_mask, dtype=np.uint8)
            
            # Assegna valori grigi basati sulle categorie semantiche
            # Usa valori che sono compatibili con SEGMENTATION_CATEGORIES
            semantic_to_gray = {
                'sky': 5,        # Valore basso per sky
                'trees': 50,     # Valore medio per trees  
                'buildings': 90, # Valore medio-alto per buildings
                'ground': 130,   # Valore alto per ground
                'water': 170,    # Valore molto alto per water
                'roads': 140,    # Valore alto per roads
                'unknown': 0     # Zero per unknown
            }
            
            # Applica mapping
            for semantic_value, category in category_mapping.items():
                if category in semantic_to_gray:
                    gray_value = semantic_to_gray[category]
                    mask = semantic_mask == semantic_value
                    seg_array[mask] = gray_value
            
            # Aggiorna SEGMENTATION_CATEGORIES dinamicamente per questa sessione
            self.update_segmentation_categories_from_semantic(semantic_to_gray)
            
            return seg_array
            
        except Exception as e:
            print(f"❌ Errore conversione semantic->segmentation: {e}")
            # Fallback: restituisci semantic_mask come array di grigi
            return semantic_mask.astype(np.uint8)
    
    def update_segmentation_categories_from_semantic(self, semantic_to_gray):
        """Aggiorna dinamicamente SEGMENTATION_CATEGORIES basandosi sulla mappatura semantica"""
        global SEGMENTATION_CATEGORIES
        
        try:
            # Crea categorie basate sui valori grigi assegnati
            updated_categories = {
                'sky': [semantic_to_gray.get('sky', 5)],
                'trees': [semantic_to_gray.get('trees', 50)],
                'buildings': [semantic_to_gray.get('buildings', 90)],
                'ground': [semantic_to_gray.get('ground', 130)] + [semantic_to_gray.get('roads', 140)],
                'obstacles': [semantic_to_gray.get('trees', 50), semantic_to_gray.get('buildings', 90)]
            }
            
            # Aggiorna solo se diverso da quello attuale
            if updated_categories != SEGMENTATION_CATEGORIES:
                SEGMENTATION_CATEGORIES.update(updated_categories)
                print(f"🔄 SEGMENTATION_CATEGORIES aggiornate per object detection:")
                for cat, values in updated_categories.items():
                    print(f"   {cat}: {values}")
                    
        except Exception as e:
            print(f"⚠️ Errore aggiornamento SEGMENTATION_CATEGORIES: {e}")
        ...
        try:
            ...
            for i, bg_color in enumerate(['black', 'white']):
                ...
                if 'sky' in SEGMENTATION_CATEGORIES:
                    for sky_val in SEGMENTATION_CATEGORIES['sky']:
                        mask &= (seg_array != sky_val)
            ...
            for i in range(2):
                ...
                for category in ['trees', 'obstacles']:
                    if category in SEGMENTATION_CATEGORIES:
                        for val in SEGMENTATION_CATEGORIES[category]:
                            mask |= (seg_array == val)
            ...
            # POSITIVI 5-6: Buildings+trees+obstacles, sfondo b_X
            for i in range(2):
    ...
    def save_anchor_set(self, anchor_img, positives, anchor_idx):
        """Salva un set completo: anchor + positivi"""
    ...
    def generate_dataset(self):
        ...
        for i in range(N_SAMPLES):
            ...
            # Cattura l'immagine anchor con segmentazione
            anchor_img, segmentation_mask = self.capture_with_segmentation()
            
            if anchor_img is None:
                print("❌ Cattura fallita")
                continue
            ...
            if self.save_anchor_set(anchor_img, positives, i):
